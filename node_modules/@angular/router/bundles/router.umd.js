...
!function (global, factory) {
...
}(undefined, function (exports, _angular_common, _angular_core, rxjs_BehaviorSubject, rxjs_Subject, rxjs_observable_from, rxjs_observable_of, rxjs_operator_concatMap, rxjs_operator_every, rxjs_operator_first, rxjs_operator_map, rxjs_operator_mergeMap, rxjs_operator_reduce, rxjs_Observable, rxjs_operator_catch, rxjs_operator_concatAll, rxjs_util_EmptyError, rxjs_observable_fromPromise, l, rxjs_operator_mergeAll, _angular_platformBrowser, rxjs_operator_filter) {
   "use strict";
   function defaultUrlMatcher(segments, segmentGroup, route) {
      for (var path = route.path, parts = path.split("/"), posParams = {}, consumed = [], currentIndex = 0, i = 0; i < parts.length; ++i) {
         if (currentIndex >= segments.length) return null;var current = segments[currentIndex],
             p = parts[i],
             isPosParam = p.startsWith(":");if (!isPosParam && p !== current.path) return null;isPosParam && (posParams[p.substring(1)] = current), consumed.push(current), currentIndex++;
      }return "full" === route.pathMatch && (segmentGroup.hasChildren() || currentIndex < segments.length) ? null : { consumed: consumed, posParams: posParams };
      ...
      var k1 = Object.keys(a),
          k2 = Object.keys(b);if (k1.length != k2.length) return !1;for (var key, i = 0; i < k1.length; i++) {
         if (key = k1[i], a[key] !== b[key]) return !1;
      }return !0;
   }function flatten(a) {
      for (var target = [], i = 0; i < a.length; ++i) {
      ...
      var waitFor = [],
          res = {};if (forEach(obj, function (a, k) {
         ...
         var concatted$ = rxjs_operator_concatAll.concatAll.call(rxjs_observable_of.of.apply(void 0, waitFor)),
             last$ = l.last.call(concatted$);return rxjs_operator_map.map.call(last$, function () {
            return res;
         });
      ...
      if (a.length !== b.length) return !1;for (var i = 0; i < a.length; ++i) {
         if (a[i].path !== b[i].path) return !1;if (!shallowEqual(a[i].parameters, b[i].parameters)) return !1;
      }return !0;
   }function equalPath(a, b) {
      if (a.length !== b.length) return !1;for (var i = 0; i < a.length; ++i) {
         if (a[i].path !== b[i].path) return !1;
      }return !0;
      ...
      var res = [];for (var prop in obj) {
         obj.hasOwnProperty(prop) && res.push(new Pair(prop, obj[prop]));
      }return res;
   }function matchSegments(str) {
      SEGMENT_RE.lastIndex = 0;var match = str.match(SEGMENT_RE);return match ? match[0] : "";
   }function matchQueryParams(str) {
      QUERY_PARAM_RE.lastIndex = 0;var match = str.match(SEGMENT_RE);return match ? match[0] : "";
   }function matchUrlQueryParamValue(str) {
      QUERY_PARAM_VALUE_RE.lastIndex = 0;var match = str.match(QUERY_PARAM_VALUE_RE);return match ? match[0] : "";
      ...
      var canLoad = route.canLoad;if (!canLoad || 0 === canLoad.length) return rxjs_observable_of.of(!0);var obs = rxjs_operator_map.map.call(rxjs_observable_from.from(canLoad), function (c) {
         var guard = injector.get(c);return wrapIntoObservable(guard.canLoad ? guard.canLoad(route) : guard(route));
      });return andObservables(obs);
   }function match(segmentGroup, route, segments) {
      var noMatch = { matched: !1, consumedSegments: [], lastChild: 0, positionalParamSegments: {} };if ("" === route.path) return "full" === route.pathMatch && (segmentGroup.hasChildren() || segments.length > 0) ? { matched: !1, consumedSegments: [], lastChild: 0, positionalParamSegments: {} } : { matched: !0, consumedSegments: [], lastChild: 0, positionalParamSegments: {} };var matcher = route.matcher || defaultUrlMatcher,
          res = matcher(segments, segmentGroup, route);return res ? { matched: !0, consumedSegments: res.consumed, lastChild: res.consumed.length, positionalParamSegments: res.posParams } : noMatch;
      ...
      for (var res = {}, _i = 0, routes_1 = routes; _i < routes_1.length; _i++) {
         var r = routes_1[_i];emptyPathRedirect(segmentGroup, slicedSegments, r) && !children[getOutlet$1(r)] && (res[getOutlet$1(r)] = new UrlSegmentGroup([], {}));
      ...
      for (var pathToRoot = route.pathFromRoot, inhertingStartingFrom = pathToRoot.length - 1; inhertingStartingFrom >= 1;) {
         var current = pathToRoot[inhertingStartingFrom],
             parent_1 = pathToRoot[inhertingStartingFrom - 1];if (current.routeConfig && "" === current.routeConfig.path) inhertingStartingFrom--;else {
            if (parent_1.component) break;inhertingStartingFrom--;
         }
      }return pathToRoot.slice(inhertingStartingFrom).reduce(function (res, curr) {
         var params = merge(res.params, curr.params),
             data = merge(res.data, curr.data),
             resolve = merge(res.resolve, curr._resolvedData);return { params: params, data: data, resolve: resolve };
      }, { params: {}, data: {}, resolve: {} });
      ...
      if (prevState && routeReuseStrategy.shouldReuseRoute(curr.value, prevState.value.snapshot)) {
         ...
         var tree = routeReuseStrategy.retrieve(curr.value).route;return setFutureSnapshotsOfActivatedRoutes(curr, tree), tree;
      }var value = createActivatedRoute(curr.value),
          children = curr.children.map(function (c) {
         return createNode(routeReuseStrategy, c);
      });return new TreeNode(value, children);
   }function setFutureSnapshotsOfActivatedRoutes(curr, result) {
      if (curr.value.routeConfig !== result.value.routeConfig) throw new Error("Cannot reattach ActivatedRouteSnapshot created from a different route");if (curr.children.length !== result.children.length) throw new Error("Cannot reattach ActivatedRouteSnapshot with a different number of children");result.value._futureSnapshot = curr.value;for (var i = 0; i < curr.children.length; ++i) {
      ...
      return "object" == (typeof command === "undefined" ? "undefined" : _typeof(command)) && !command.outlets && !command.segmentPath;
      ...
      if ("string" == typeof commands[0] && 1 === commands.length && "/" === commands[0]) return new Navigation(!0, 0, commands);var numberOfDoubleDots = 0,
         ...
         if ("object" == (typeof cmd === "undefined" ? "undefined" : _typeof(cmd))) {
            if (cmd.outlets) {
      ...
      return "object" == (typeof command === "undefined" ? "undefined" : _typeof(command)) && command.outlets ? command.outlets[PRIMARY_OUTLET] : "" + command;
   }function getOutlets(commands) {
      return "object" != _typeof(commands[0]) ? (_a = {}, _a[PRIMARY_OUTLET] = commands, _a) : void 0 === commands[0].outlets ? (_b = {}, _b[PRIMARY_OUTLET] = commands, _b) : commands[0].outlets;var _a, _b;
      ...
      if (0 === commands.length) return new UrlSegmentGroup(segmentGroup.segments, {});var outlets_2 = getOutlets(commands),
          children_1 = {};return forEach(outlets_2, function (commands, outlet) {
      ...
      for (var currentCommandIndex = 0, currentPathIndex = startIndex, noMatch = { match: !1, pathIndex: 0, commandIndex: 0 }; currentPathIndex < segmentGroup.segments.length;) {
         if (currentCommandIndex >= commands.length) return noMatch;var path = segmentGroup.segments[currentPathIndex],
             ...
             next = currentCommandIndex < commands.length - 1 ? commands[currentCommandIndex + 1] : null;if (currentPathIndex > 0 && void 0 === curr) break;if (curr && next && "object" == (typeof next === "undefined" ? "undefined" : _typeof(next)) && void 0 === next.outlets) {
      ...
      for (var paths = segmentGroup.segments.slice(0, startIndex), i = 0; i < commands.length;) {
         if ("object" == _typeof(commands[i]) && void 0 !== commands[i].outlets) {
      ...
      if ("" === route.path) {
         ...
         posParams[k] = v.path;
      });var parameters = merge(posParams, res.consumed[res.consumed.length - 1].parameters);return { consumedSegments: res.consumed, lastChild: res.consumed.length, parameters: parameters };
      ...
      for (var s = segmentGroup, res = s._segmentIndexShift ? s._segmentIndexShift : 0; s._sourceSegment;) {
         s = s._sourceSegment, res += s._segmentIndexShift ? s._segmentIndexShift : 0;
      }return res - 1;
   }function split$1(segmentGroup, consumedSegments, slicedSegments, config) {
      if (slicedSegments.length > 0 && containsEmptyPathMatchesWithNamedOutlets(segmentGroup, slicedSegments, config)) {
         var s = new UrlSegmentGroup(consumedSegments, createChildrenForEmptyPaths(segmentGroup, consumedSegments, config, new UrlSegmentGroup(slicedSegments, segmentGroup.children)));return s._sourceSegment = segmentGroup, s._segmentIndexShift = consumedSegments.length, { segmentGroup: s, slicedSegments: [] };
      }if (0 === slicedSegments.length && containsEmptyPathMatches(segmentGroup, slicedSegments, config)) {
         var s = new UrlSegmentGroup(segmentGroup.segments, addEmptyPathsToChildrenIfNeeded(segmentGroup, slicedSegments, config, segmentGroup.children));return s._sourceSegment = segmentGroup, s._segmentIndexShift = consumedSegments.length, { segmentGroup: s, slicedSegments: slicedSegments };
      ...
      for (var res = {}, _i = 0, routes_1 = routes; _i < routes_1.length; _i++) {
         var r = routes_1[_i];if (emptyPathMatch(segmentGroup, slicedSegments, r) && !children[getOutlet$2(r)]) {
      ...
      var res = {};res[PRIMARY_OUTLET] = primarySegment, primarySegment._sourceSegment = segmentGroup, primarySegment._segmentIndexShift = consumedSegments.length;for (var _i = 0, routes_2 = routes; _i < routes_2.length; _i++) {
         var r = routes_2[_i];if ("" === r.path && getOutlet$2(r) !== PRIMARY_OUTLET) {
      ...
      for (var s = snapshot.parent; s;) {
         var c = s._routeConfig;if (c && c._loadedConfig) return c._loadedConfig;if (c && c.component) return null;s = s.parent;
      }return null;
   }function closestLoadedConfig(snapshot) {
      if (!snapshot) return null;for (var s = snapshot.parent; s;) {
         var c = s._routeConfig;if (c && c._loadedConfig) return c._loadedConfig;s = s.parent;
      }return null;
      ...
      void 0 === opts && (opts = {});var router = new Router(null, urlSerializer, outletMap, location, injector, loader, compiler, flatten(config));if (urlHandlingStrategy && (router.urlHandlingStrategy = urlHandlingStrategy), routeReuseStrategy && (router.routeReuseStrategy = routeReuseStrategy), opts.errorHandler && (router.errorHandler = opts.errorHandler), opts.enableTracing) {
         var dom_1 = getDOM();router.events.subscribe(function (e) {
      ...
      return [{ provide: ROUTER_INITIALIZER, useFactory: initialRouterNavigation, deps: [Router, _angular_core.ApplicationRef, RouterPreloader, ROUTER_CONFIGURATION] }, { provide: _angular_core.APP_BOOTSTRAP_LISTENER, multi: !0, useExisting: ROUTER_INITIALIZER }];
   } /**
     * @license
   ...
   var __extends = this && this.__extends || function (d, b) {
      ...
      function RouterConfigLoader(loader, compiler) {
         ...
         return rxjs_operator_map.map.call(this.loadModuleFactory(loadChildren), function (r) {
            var ref = r.create(parentInjector),
                injectorFactory = function injectorFactory(parent) {
         ...
         var _this = this;if ("string" == typeof loadChildren) return rxjs_observable_fromPromise.fromPromise(this.loader.load(loadChildren));var offlineMode_1 = this.compiler instanceof _angular_core.Compiler;return rxjs_operator_mergeMap.mergeMap.call(wrapIntoObservable(loadChildren()), function (t) {
      ...
      }, Object.defineProperty(UrlSegmentGroup.prototype, "numberOfChildren", { get: function get() {
      ...
      function DefaultUrlSerializer() {}return DefaultUrlSerializer.prototype.parse = function (url) {
         ...
         var segment = "/" + serializeSegment(tree.root, !0),
             ...
             fragment = null !== tree.fragment && void 0 !== tree.fragment ? "#" + encodeURI(tree.fragment) : "";return "" + segment + query + fragment;
      ...
      function UrlParser(url) {
         ...
         if (!this.remaining.startsWith(str)) throw new Error('Expected "' + str + '".');this.remaining = this.remaining.substring(str.length);
         ...
         if (0 == this.remaining.length) return {};this.peekStartsWith("/") && this.capture("/");var paths = [];for (this.peekStartsWith("(") || paths.push(this.parseSegments()); this.peekStartsWith("/") && !this.peekStartsWith("//") && !this.peekStartsWith("/(");) {
            this.capture("/"), paths.push(this.parseSegments());
         }var children = {};this.peekStartsWith("/(") && (this.capture("/"), children = this.parseParens(!0));var res = {};return this.peekStartsWith("(") && (res = this.parseParens(!1)), (paths.length > 0 || Object.keys(children).length > 0) && (res[PRIMARY_OUTLET] = new UrlSegmentGroup(paths, children)), res;
      }, UrlParser.prototype.parseSegments = function () {
         var path = matchSegments(this.remaining);if ("" === path && this.peekStartsWith(";")) throw new Error("Empty path url segment cannot have parameters: '" + this.remaining + "'.");this.capture(path);var matrixParams = {};return this.peekStartsWith(";") && (matrixParams = this.parseMatrixParams()), new UrlSegment(decode(path), matrixParams);
      }, UrlParser.prototype.parseQueryParams = function () {
         var params = {};if (this.peekStartsWith("?")) for (this.capture("?"), this.parseQueryParam(params); this.remaining.length > 0 && this.peekStartsWith("&");) {
         ...
         var key = matchSegments(this.remaining);if (key) {
            this.capture(key);var value = "";if (this.peekStartsWith("=")) {
               this.capture("=");var valueMatch = matchSegments(this.remaining);valueMatch && (value = valueMatch, this.capture(value));
            }params[decode(key)] = decode(value);
         }
      }, UrlParser.prototype.parseQueryParam = function (params) {
         var key = matchQueryParams(this.remaining);if (key) {
            this.capture(key);var value = "";if (this.peekStartsWith("=")) {
               this.capture("=");var valueMatch = matchUrlQueryParamValue(this.remaining);valueMatch && (value = valueMatch, this.capture(value));
      ...
      function ApplyRedirects(injector, configLoader, urlSerializer, urlTree, config) {
         this.injector = injector, this.configLoader = configLoader, this.urlSerializer = urlSerializer, this.urlTree = urlTree, this.config = config, this.allowRedirects = !0;
         ...
         var _this = this,
            ...
            if (e instanceof AbsoluteRedirect) return _this.allowRedirects = !1, _this.match(e.urlTree);throw e instanceof NoMatch ? _this.noMatchError(e) : e;
         });
      }, ApplyRedirects.prototype.match = function (tree) {
         var _this = this,
             expanded$ = this.expandSegmentGroup(this.injector, this.config, tree.root, PRIMARY_OUTLET),
         ...
         var _this = this,
            ...
            var expanded$ = _this.expandSegmentAgainstRoute(injector, segmentGroup, routes, r, segments, outlet, allowRedirects);return rxjs_operator_catch._catch.call(expanded$, function (e) {
         ...
         var _this = this,
            ...
            var group = new UrlSegmentGroup(newSegments, {});return _this.expandSegment(injector, group, routes, newSegments, outlet, !1);
         });
         ...
         var _this = this,
            ...
            return _this.expandSegment(injector, segmentGroup, routes, newSegments.concat(segments.slice(lastChild)), outlet, !1);
         });
      }, ApplyRedirects.prototype.matchSegmentAgainstRoute = function (injector, rawSegmentGroup, route, segments) {
         var _this = this;if ("**" === route.path) return route.loadChildren ? rxjs_operator_map.map.call(this.configLoader.load(injector, route.loadChildren), function (r) {
            return route._loadedConfig = r, rxjs_observable_of.of(new UrlSegmentGroup(segments, {}));
             ...
             lastChild = _a.lastChild;if (!matched) return noMatch(rawSegmentGroup);var rawSlicedSegments_1 = segments.slice(lastChild),
             childConfig$ = this.getChildConfig(injector, route);return rxjs_operator_mergeMap.mergeMap.call(childConfig$, function (routerConfig) {
            var childInjector = routerConfig.injector,
               ...
               var expanded$ = _this.expandChildren(childInjector, childConfig, segmentGroup);return rxjs_operator_map.map.call(expanded$, function (children) {
                  return new UrlSegmentGroup(consumedSegments_1, children);
               });
            }if (0 === childConfig.length && 0 === slicedSegments.length) return rxjs_observable_of.of(new UrlSegmentGroup(consumedSegments_1, {}));var expanded$ = _this.expandSegment(childInjector, segmentGroup, childConfig, slicedSegments, PRIMARY_OUTLET, !0);return rxjs_operator_map.map.call(expanded$, function (cs) {
               return new UrlSegmentGroup(consumedSegments_1.concat(cs.segments), cs.children);
            });
         });
      }, ApplyRedirects.prototype.getChildConfig = function (injector, route) {
         var _this = this;return route.children ? rxjs_observable_of.of(new LoadedRouterConfig(route.children, injector, null, null)) : route.loadChildren ? rxjs_operator_mergeMap.mergeMap.call(runGuards(injector, route), function (shouldLoad) {
         ...
         this.urlSerializer.parse(redirectTo);return this.applyRedirectCreatreUrlTree(redirectTo, this.urlSerializer.parse(redirectTo), segments, posParams);
         ...
         var res = {};return forEach(redirectToParams, function (v, k) {
            v.startsWith(":") ? res[k] = actualParams[v.substring(1)] : res[k] = v;
         }), res;
      ...
      function Tree(root) {
         this._root = root;
      }return Object.defineProperty(Tree.prototype, "root", { get: function get() {
      ...
      }return Object.defineProperty(ActivatedRoute.prototype, "routeConfig", { get: function get() {
            return this._futureSnapshot.routeConfig;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRoute.prototype, "root", { get: function get() {
            return this._routerState.root;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRoute.prototype, "parent", { get: function get() {
            return this._routerState.parent(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRoute.prototype, "firstChild", { get: function get() {
            return this._routerState.firstChild(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRoute.prototype, "children", { get: function get() {
            return this._routerState.children(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRoute.prototype, "pathFromRoot", { get: function get() {
      ...
      }return Object.defineProperty(ActivatedRouteSnapshot.prototype, "routeConfig", { get: function get() {
            return this._routeConfig;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRouteSnapshot.prototype, "root", { get: function get() {
            return this._routerState.root;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRouteSnapshot.prototype, "parent", { get: function get() {
            return this._routerState.parent(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRouteSnapshot.prototype, "firstChild", { get: function get() {
            return this._routerState.firstChild(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRouteSnapshot.prototype, "children", { get: function get() {
            return this._routerState.children(this);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(ActivatedRouteSnapshot.prototype, "pathFromRoot", { get: function get() {
            return this._routerState.pathFromRoot(this);
      ...
      function Navigation(isAbsolute, numberOfDoubleDots, commands) {
         if (this.isAbsolute = isAbsolute, this.numberOfDoubleDots = numberOfDoubleDots, this.commands = commands, isAbsolute && commands.length > 0 && isMatrixParams(commands[0])) throw new Error("Root segment cannot have matrix parameters");var cmdWithOutlet = commands.find(function (c) {
            return "object" == (typeof c === "undefined" ? "undefined" : _typeof(c)) && c.outlets;
         });if (cmdWithOutlet && cmdWithOutlet !== last(commands)) throw new Error("{outlets:{}} has to be the last command");
      ...
      function Position(segmentGroup, processChildren, index) {
       ...
       NoMatch$1 = function () {
      function NoMatch() {}return NoMatch;
   }(),
       Recognizer = function () {
      function Recognizer(rootComponentType, config, urlTree, url) {
         ...
         if (route.redirectTo) throw new NoMatch$1();if ((route.outlet ? route.outlet : PRIMARY_OUTLET) !== outlet) throw new NoMatch$1();if ("**" === route.path) {
            ...
            var children = this.processChildren(childConfig, segmentGroup);return [new TreeNode(snapshot, children)];
      ...
      function Router(rootComponentType, urlSerializer, outletMap, location, injector, loader, compiler, config) {
      ...
      }, Object.defineProperty(Router.prototype, "routerState", { get: function get() {
            return this.currentRouterState;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(Router.prototype, "url", { get: function get() {
            return this.serializeUrl(this.currentUrlTree);
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(Router.prototype, "events", { get: function get() {
            return this.routerEvents;
         }, enumerable: !0, configurable: !0 }), Router.prototype.resetConfig = function (config) {
         ...
         if (void 0 === extras && (extras = { skipLocationChange: !1 }), url instanceof UrlTree) return this.scheduleNavigation(this.urlHandlingStrategy.merge(url, this.rawUrlTree), "imperative", extras);var urlTree = this.urlSerializer.parse(url);return this.scheduleNavigation(this.urlHandlingStrategy.merge(urlTree, this.rawUrlTree), "imperative", extras);
      }, Router.prototype.navigate = function (commands, extras) {
         return void 0 === extras && (extras = { skipLocationChange: !1 }), "object" == _typeof(extras.queryParams) && null !== extras.queryParams && (extras.queryParams = this.removeEmptyProps(extras.queryParams)), this.navigateByUrl(this.createUrlTree(commands, extras), extras);
         ...
         var lastNavigation = this.navigations.value;if (lastNavigation && "imperative" !== source && "imperative" === lastNavigation.source && lastNavigation.rawUrl.toString() === rawUrl.toString()) return null;if (lastNavigation && "hashchange" == source && "popstate" === lastNavigation.source && lastNavigation.rawUrl.toString() === rawUrl.toString()) return null;var resolve = null,
             reject = null,
         ...
         var _this = this;return id !== this.navigationId ? (this.location.go(this.urlSerializer.serialize(this.currentUrlTree)), this.routerEvents.next(new NavigationCancel(id, this.serializeUrl(url), "Navigation ID " + id + " is not equal to the current navigation id " + this.navigationId)), Promise.resolve(!1)) : new Promise(function (resolvePromise, rejectPromise) {
            var urlAndSnapshot$;if (precreatedState) urlAndSnapshot$ = rxjs_observable_of.of({ appliedUrl: url, snapshot: precreatedState });else {
               var redirectsApplied$ = applyRedirects(_this.injector, _this.configLoader, _this.urlSerializer, url, _this.config);urlAndSnapshot$ = rxjs_operator_mergeMap.mergeMap.call(redirectsApplied$, function (appliedUrl) {
                ...
                preactivationTraverse$ = rxjs_operator_map.map.call(urlAndSnapshot$, function (_a) {
               var appliedUrl = _a.appliedUrl,
                   snapshot = _a.snapshot;return preActivation = new PreActivation(snapshot, _this.currentRouterState.snapshot, _this.injector), preActivation.traverse(_this.outletMap), { appliedUrl: appliedUrl, snapshot: snapshot };
            }),
               ...
               return _this.navigationId !== id ? rxjs_observable_of.of(!1) : p.shouldActivate ? rxjs_operator_map.map.call(preActivation.resolveData(), function () {
                ...
                routerState$ = rxjs_operator_map.map.call(preactivationResolveData$, function (_a) {
               var appliedUrl = _a.appliedUrl,
               ...
               _this.navigated = !0, navigationIsSuccessful ? (_this.routerEvents.next(new NavigationEnd(id, _this.serializeUrl(url), _this.serializeUrl(_this.currentUrlTree))), resolvePromise(!0)) : (_this.resetUrlToCurrentUrlTree(), _this.routerEvents.next(new NavigationCancel(id, _this.serializeUrl(url), "")), resolvePromise(!1));
            }, function (e) {
               if (e instanceof NavigationCancelingError) _this.resetUrlToCurrentUrlTree(), _this.navigated = !0, _this.routerEvents.next(new NavigationCancel(id, _this.serializeUrl(url), e.message)), resolvePromise(!1);else {
      ...
      function CanActivate(path) {
         this.path = path;
      }return Object.defineProperty(CanActivate.prototype, "route", { get: function get() {
      ...
      function PreActivation(future, curr, injector) {
         ...
         var _this = this;if (0 === this.checks.length) return rxjs_observable_of.of(!0);var checks$ = rxjs_observable_from.from(this.checks),
             runningChecks$ = rxjs_operator_mergeMap.mergeMap.call(checks$, function (s) {
            if (s instanceof CanActivate) return andObservables(rxjs_observable_from.from([_this.runCanActivateChild(s.path), _this.runCanActivate(s.route)]));if (s instanceof CanDeactivate) {
               var s2 = s;return _this.runCanDeactivate(s2.component, s2.route);
            }throw new Error("Cannot be reached");
         });return rxjs_operator_every.every.call(runningChecks$, function (result) {
            return result === !0;
         });
      }, PreActivation.prototype.resolveData = function () {
         var _this = this;if (0 === this.checks.length) return rxjs_observable_of.of(null);var checks$ = rxjs_observable_from.from(this.checks),
         ...
         var _this = this,
            ...
            var obs = rxjs_operator_map.map.call(rxjs_observable_from.from(d.guards), function (c) {
               var observable,
                   guard = _this.getToken(c, c.node);return observable = wrapIntoObservable(guard.canActivateChild ? guard.canActivateChild(future, _this.future) : guard(future, _this.future)), rxjs_operator_first.first.call(observable);
            });return andObservables(obs);
         }));
         ...
         var _this = this,
            ...
            var observable,
                guard = _this.getToken(c, curr);return observable = wrapIntoObservable(guard.canDeactivate ? guard.canDeactivate(component, curr, _this.curr) : guard(component, curr, _this.curr)), rxjs_operator_first.first.call(observable);
         ...
         var config = closestLoadedConfig(snapshot),
             injector = config ? config.injector : this.injector;return injector.get(token);
      }, PreActivation;
   }(),
      ...
      function RouterLink(router, route) {
         this.router = router, this.route = route, this.commands = [];
      }return Object.defineProperty(RouterLink.prototype, "routerLink", { set: function set(data) {
      ...
      }, Object.defineProperty(RouterLink.prototype, "urlTree", { get: function get() {
            return this.router.createUrlTree(this.commands, { relativeTo: this.route, queryParams: this.queryParams, fragment: this.fragment, preserveQueryParams: attrBoolValue(this.preserveQueryParams), preserveFragment: attrBoolValue(this.preserveFragment) });
         }, enumerable: !0, configurable: !0 }), RouterLink.decorators = [{ type: _angular_core.Directive, args: [{ selector: ":not(a)[routerLink]" }] }], RouterLink.ctorParameters = function () {
         return [{ type: Router }, { type: ActivatedRoute }];
      }, RouterLink.propDecorators = { queryParams: [{ type: _angular_core.Input }], fragment: [{ type: _angular_core.Input }], preserveQueryParams: [{ type: _angular_core.Input }], preserveFragment: [{ type: _angular_core.Input }], skipLocationChange: [{ type: _angular_core.Input }], replaceUrl: [{ type: _angular_core.Input }], routerLink: [{ type: _angular_core.Input }], onClick: [{ type: _angular_core.HostListener, args: ["click", []] }] }, RouterLink;
   }(),
      ...
      }return Object.defineProperty(RouterLinkWithHref.prototype, "routerLink", { set: function set(data) {
            Array.isArray(data) ? this.commands = data : this.commands = [data];
      ...
      }, Object.defineProperty(RouterLinkWithHref.prototype, "urlTree", { get: function get() {
            return this.router.createUrlTree(this.commands, { relativeTo: this.route, queryParams: this.queryParams, fragment: this.fragment, preserveQueryParams: attrBoolValue(this.preserveQueryParams), preserveFragment: attrBoolValue(this.preserveFragment) });
         }, enumerable: !0, configurable: !0 }), RouterLinkWithHref.decorators = [{ type: _angular_core.Directive, args: [{ selector: "a[routerLink]" }] }], RouterLinkWithHref.ctorParameters = function () {
         return [{ type: Router }, { type: ActivatedRoute }, { type: _angular_common.LocationStrategy }];
      }, RouterLinkWithHref.propDecorators = { target: [{ type: _angular_core.Input }], queryParams: [{ type: _angular_core.Input }], fragment: [{ type: _angular_core.Input }], preserveQueryParams: [{ type: _angular_core.Input }], preserveFragment: [{ type: _angular_core.Input }], skipLocationChange: [{ type: _angular_core.Input }], replaceUrl: [{ type: _angular_core.Input }], href: [{ type: _angular_core.HostBinding }], routerLink: [{ type: _angular_core.Input }], onClick: [{ type: _angular_core.HostListener, args: ["click", ["$event.button", "$event.ctrlKey", "$event.metaKey"]] }] }, RouterLinkWithHref;
   }(),
       RouterLinkActive = function () {
      function RouterLinkActive(router, element, renderer) {
         var _this = this;this.router = router, this.element = element, this.renderer = renderer, this.classes = [], this.routerLinkActiveOptions = { exact: !1 }, this.subscription = router.events.subscribe(function (s) {
            s instanceof NavigationEnd && _this.update();
         });
      }return Object.defineProperty(RouterLinkActive.prototype, "isActive", { get: function get() {
            return this.hasActiveLink();
         }, enumerable: !0, configurable: !0 }), RouterLinkActive.prototype.ngAfterContentInit = function () {
         var _this = this;this.links.changes.subscribe(function (s) {
            return _this.update();
         }), this.linksWithHrefs.changes.subscribe(function (s) {
            return _this.update();
         }), this.update();
      }, Object.defineProperty(RouterLinkActive.prototype, "routerLinkActive", { set: function set(data) {
         ...
         var _this = this;if (this.links && this.linksWithHrefs && this.router.navigated) {
            var isActive = this.hasActiveLink();this.classes.forEach(function (c) {
      ...
      }, RouterLinkActive.propDecorators = { links: [{ type: _angular_core.ContentChildren, args: [RouterLink, { descendants: !0 }] }], linksWithHrefs: [{ type: _angular_core.ContentChildren, args: [RouterLinkWithHref, { descendants: !0 }] }], routerLinkActiveOptions: [{ type: _angular_core.Input }], routerLinkActive: [{ type: _angular_core.Input }] }, RouterLinkActive;
   }(),
      ...
      }, Object.defineProperty(RouterOutlet.prototype, "locationInjector", { get: function get() {
            return this.location.injector;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(RouterOutlet.prototype, "locationFactoryResolver", { get: function get() {
            return this.resolver;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(RouterOutlet.prototype, "isActivated", { get: function get() {
            return !!this.activated;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(RouterOutlet.prototype, "component", { get: function get() {
            if (!this.activated) throw new Error("Outlet is not activated");return this.activated.instance;
         }, enumerable: !0, configurable: !0 }), Object.defineProperty(RouterOutlet.prototype, "activatedRoute", { get: function get() {
      ...
      }, RouterOutlet.propDecorators = { activateEvents: [{ type: _angular_core.Output, args: ["activate"] }], deactivateEvents: [{ type: _angular_core.Output, args: ["deactivate"] }] }, RouterOutlet;
   }(),
      ...
      function RouterPreloader(router, moduleLoader, compiler, injector, preloadingStrategy) {
         ...
         for (var res = [], _i = 0, routes_1 = routes; _i < routes_1.length; _i++) {
            var c = routes_1[_i];if (c.loadChildren && !c.canLoad && c._loadedConfig) {
               var childConfig = c._loadedConfig;res.push(this.processRoutes(childConfig.injector, childConfig.routes));
            } else c.loadChildren && !c.canLoad ? res.push(this.preloadConfig(injector, c)) : c.children && res.push(this.processRoutes(injector, c.children));
         ...
         var _this = this;return this.preloadingStrategy.preload(route, function () {
            var loaded = _this.loader.load(injector, route.loadChildren);return rxjs_operator_mergeMap.mergeMap.call(loaded, function (config) {
               var c = route;return c._loadedConfig = config, _this.processRoutes(config.injector, config.routes);
            });
         });
      ...
      function RouterModule(guard) {}return RouterModule.forRoot = function (routes, config) {
         ...
         return [{ type: void 0, decorators: [{ type: _angular_core.Optional
            }, { type: _angular_core.Inject, args: [ROUTER_FORROOT_GUARD] }] }];
      }, RouterModule;
   }(),
       ROUTER_INITIALIZER = new _angular_core.OpaqueToken("Router Initializer"),
       VERSION = new _angular_core.Version("3.4.1"),
