...
!function (global, factory) {
  ...
  function normalizeMethodName(method) {
    ...
    for (var view = new Uint16Array(input.length), i = 0, strLen = input.length; i < strLen; i++) {
  ...
  function paramParser(rawParams) {
    ...
    var w = "object" == (typeof window === "undefined" ? "undefined" : _typeof(window)) ? window : {};return null === _jsonpConnections && (_jsonpConnections = w[JSONP_HOME] = {}), _jsonpConnections;
      ...
      var headers = new Headers();return headersString.split("\n").forEach(function (line) {
        var index = line.indexOf(":");if (index > 0) {
          var name_1 = line.slice(0, index),
              value = line.slice(index + 1).trim();headers.set(name_1, value);
        }
      }), headers;
    ...
    }return __extends$1(BaseResponseOptions, _super), BaseResponseOptions.decorators = [{ type: _angular_core.Injectable }], BaseResponseOptions.ctorParameters = function () {
      return [];
    }, BaseResponseOptions;
  }(ResponseOptions),
    ...
    function URLSearchParams(rawParams, queryEncoder) {
      ...
      var _this = this;searchParams.paramsMap.forEach(function (value, param) {
        for (var list = _this.paramsMap.get(param) || [], i = 0; i < value.length; ++i) {
          list.push(value[i]);
        }_this.paramsMap.set(param, list);
      });
    }, URLSearchParams.prototype.replaceAll = function (searchParams) {
      var _this = this;searchParams.paramsMap.forEach(function (value, param) {
        var list = _this.paramsMap.get(param) || [];list.length = 0;for (var i = 0; i < value.length; ++i) {
          list.push(value[i]);
        }_this.paramsMap.set(param, list);
      });
    ...
    function Body() {}return Body.prototype.json = function () {
      ...
      return this._body instanceof URLSearchParams ? this._body.toString() : this._body instanceof ArrayBuffer ? String.fromCharCode.apply(null, new Uint16Array(this._body)) : null == this._body ? "" : "object" == _typeof(this._body) ? JSON.stringify(this._body, null, 2) : this._body.toString();
    ...
    function Response(responseOptions) {
      _super.call(this), this._body = responseOptions.body, this.status = responseOptions.status, this.ok = this.status >= 200 && this.status <= 299, this.statusText = responseOptions.statusText, this.headers = responseOptions.headers, this.type = responseOptions.type, this.url = responseOptions.url;
    }return __extends$2(Response, _super), Response.prototype.toString = function () {
    ...
    function JSONPConnection_(req, _dom, baseResponseOptions) {
      var _this = this;if (_super.call(this), this._dom = _dom, this.baseResponseOptions = baseResponseOptions, this._finished = !1, req.method !== RequestMethod.Get) throw new TypeError(JSONP_ERR_WRONG_METHOD);this.request = req, this.response = new rxjs_Observable.Observable(function (responseObserver) {
    ...
    }, JSONPBackend_.decorators = [{ type: _angular_core.Injectable }], JSONPBackend_.ctorParameters = function () {
    ...
    function XHRConnection(req, browserXHR, baseResponseOptions) {
      var _this = this;this.request = req, this.response = new rxjs_Observable.Observable(function (responseObserver) {
        var _xhr = browserXHR.build();_xhr.open(RequestMethod[req.method].toUpperCase(), req.url), null != req.withCredentials && (_xhr.withCredentials = req.withCredentials);var onLoad = function onLoad() {
    ...
    function CookieXSRFStrategy(_cookieName, _headerName) {
      ...
      var xsrfToken = _angular_platformBrowser.__platform_browser_private__.getDOM().getCookie(this._cookieName);xsrfToken && req.headers.set(this._headerName, xsrfToken);
    }, CookieXSRFStrategy;
  }(),
      XHRBackend = function () {
    ...
    function RequestOptions(_a) {
      var _b = void 0 === _a ? {} : _a,
          ...
          responseType = _b.responseType;this.method = null != method ? normalizeMethodName(method) : null, this.headers = null != headers ? headers : null, this.body = null != body ? body : null, this.url = null != url ? url : null, this.search = null != search ? "string" == typeof search ? new URLSearchParams(search) : search : null, this.withCredentials = null != withCredentials ? withCredentials : null, this.responseType = null != responseType ? responseType : null;
    }return RequestOptions.prototype.merge = function (options) {
      return new RequestOptions({ method: options && null != options.method ? options.method : this.method, headers: options && null != options.headers ? options.headers : new Headers(this.headers), body: options && null != options.body ? options.body : this.body, url: options && null != options.url ? options.url : this.url, search: options && null != options.search ? "string" == typeof options.search ? new URLSearchParams(options.search) : options.search.clone() : this.search, withCredentials: options && null != options.withCredentials ? options.withCredentials : this.withCredentials, responseType: options && null != options.responseType ? options.responseType : this.responseType });
    }, RequestOptions;
  }(),
      BaseRequestOptions = function (_super) {
    function BaseRequestOptions() {
      _super.call(this, { method: RequestMethod.Get, headers: new Headers() });
    }return __extends$3(BaseRequestOptions, _super), BaseRequestOptions.decorators = [{ type: _angular_core.Injectable }], BaseRequestOptions.ctorParameters = function () {
      return [];
    }, BaseRequestOptions;
  }(RequestOptions),
    ...
    function Request(requestOptions) {
      _super.call(this);var url = requestOptions.url;if (this.url = requestOptions.url, requestOptions.search) {
        var search = requestOptions.search.toString();if (search.length > 0) {
          var prefix = "?";this.url.indexOf("?") != -1 && (prefix = "&" == this.url[this.url.length - 1] ? "" : "&"), this.url = url + prefix + search;
        }
    ...
    }return __extends$5(Request, _super), Request.prototype.detectContentType = function () {
      switch (this.headers.get("content-type")) {case "application/json":
          ...
          return ContentType.BLOB;default:
          return this.detectContentTypeFromBody();}
    }, Request.prototype.detectContentTypeFromBody = function () {
      return null == this._body ? ContentType.NONE : this._body instanceof URLSearchParams ? ContentType.FORM : this._body instanceof FormData ? ContentType.FORM_DATA : this._body instanceof Blob$1 ? ContentType.BLOB : this._body instanceof ArrayBuffer$1 ? ContentType.ARRAY_BUFFER : this._body && "object" == _typeof(this._body) ? ContentType.JSON : ContentType.TEXT;
      ...
      noop = function noop() {},
      w = "object" == (typeof window === "undefined" ? "undefined" : _typeof(window)) ? window : noop,
    ...
    }return __extends$4(Jsonp, _super), Jsonp.prototype.request = function (url, options) {
